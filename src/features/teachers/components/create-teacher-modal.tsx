...
import {
  User, Mail, LockKeyhole, Eye as EyeIcon, EyeOff, CheckCircle2, AlertCircle, X
} from "lucide-react";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { Label } from "@/components/ui/label";
import { Card, CardContent, CardHeader, CardTitle, CardDescription } from "@/components/ui/card";
import { useCreateTeacher } from "@/features/teachers/services";
...
export function CreateTeacherModal({ onClose }: { onClose: () => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Update local storage so they show in the table
      const stored = localStorage.getItem("radora_teachers");
      const teachersList = stored ? JSON.parse(stored) : [];
      ...
      const newTeacher = {
        ...
        experience: "1 Year",
        status: "active",
        courses: [], // no assigned classes initially
      };

      localStorage.setItem("radora_teachers", JSON.stringify([newTeacher, ...teachersList]));

      // Notify other tabs/components
      window.dispatchEvent(new Event("storage"));

      setSuccess({ name: name.trim(), email: email.trim().toLowerCase() });
    } catch (err: any) {
        ...
        <Card className="shadow-2xl border-0">
          <CardHeader className="pb-4 border-b">
            <div className="flex items-center justify-between">
              <div>
                <CardTitle className="text-lg">Create Teacher Account</CardTitle>
                <CardDescription className="text-sm mt-0.5">
                  Teacher can use their email to log in.
                </CardDescription>
              </div>
              <button
                onClick={onClose}
            ...
            {success ? (
              ...
              >
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground mt-1">
                    Teacher <span className="font-medium text-slate-700">{success.name}</span> can now log in with:
                  </p>
                </div>
                <div className="rounded-xl bg-slate-50 border p-4 text-left space-y-2">
                  <div className="flex justify-between text-sm">
                    <span className="text-muted-foreground">Email</span>
                    <span className="font-medium text-indigo-700">{success.email}</span>
                  </div>
                  <div className="flex justify-between text-sm">
                  ...
                  <div className="flex justify-between text-sm">
                    ...
                    <span className="font-medium">Teacher Tab on Login</span>
                  </div>
                </div>
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="space-y-1.5">
                  ...
                  <p className="text-xs text-muted-foreground">Used as the login identifier.</p>
                </div>

                {/* Password */}
                <div className="space-y-1.5">
                  <Label htmlFor="t-pwd">Login Password</Label>
                  <div className="relative">
