...
import {
  ArrowLeft, Mail, MapPin, Loader2, BookOpen, Users,
  GraduationCap, Pencil, Check, X, AlertCircle,
} from "lucide-react";
import { useAuth } from "@/features/auth/context/auth-context";
import { useTeacherProfile, useUpdateTeacherProfile } from "@/features/teacher/services/teacher-profile.service";
      ...
      <div className="flex items-start justify-between">
        <div className="flex items-center gap-4">
          ...
          <Avatar className="h-16 w-16 shadow-sm">
            <AvatarImage src={user?.image || ""} alt={profile.name} className="object-cover" />
          ...
          <div>
            ...
            <p className="text-sm text-slate-500 mt-0.5">
              Employee ID: {profile.id.substring(0, 8).toUpperCase()}
            </p>
          </div>
        </div>
      </div>
        ...
        <Card className="p-4 border-blue-100 bg-gradient-to-br from-blue-50 to-white shadow-sm">
          <div className="flex items-center gap-3">
            ...
            <div className="overflow-hidden">
              ...
              <p className="text-sm font-bold text-blue-900 truncate">{profile.email}</p>
            </div>
          </div>
        </Card>
        <Card className="p-4 border-indigo-100 bg-gradient-to-br from-indigo-50 to-white shadow-sm">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-lg font-bold text-indigo-900">{profile.courses.length}</p>
            </div>
          </div>
        </Card>
                        ...
                        <div className="flex items-start gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-sm text-slate-500">
                              ...
                              {(course._count?.enrollments || 0) !== 1 ? "s" : ""} enrolled
                            </p>
                          </div>
                        </div>
                        <Badge variant="outline" className="bg-white text-slate-600 shrink-0">
                          Active
                        </Badge>
                      </div>
                    );
