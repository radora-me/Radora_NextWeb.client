  ...
  return (
    ...
    <div className="flex flex-col h-[calc(100vh-5rem)] w-full min-h-0">
      <PageHeader
        ...
        description="Discuss assignments, ask questions, and connect with your class."
      />
      ...
      <div className="flex flex-1 min-h-0 rounded-xl border border-indigo-100 shadow-sm bg-white overflow-hidden mt-3">
        ...
        >
          ...
          <div className="flex items-center gap-2 px-4 py-3.5 border-b border-slate-100 bg-white shrink-0">
            ...
            <h2 className="font-semibold text-sm text-slate-800">My Classes</h2>
          </div>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {activeRoom && (
                    <p className="text-xs text-emerald-600 flex items-center gap-1 mt-0.5">
                      ...
                      {activeRoom.teacherName} · {activeRoom.memberCount} Participants
                    </p>
                  )}
                </div>
              </>
            ) : (
              <p className="text-sm text-slate-400">Select a class to start chatting</p>
            )}
          </div>

          {/* Messages — scrollable area with min-h-0 to allow shrinking */}
          <div className="flex-1 overflow-y-auto bg-slate-50/40 px-4 py-4 min-h-0">
            {!activeRoomId ? (
              <div className="flex items-center justify-center h-full text-sm text-slate-400">
                Select a class from the sidebar to start chatting.
              </div>
            ) : messagesLoading ? (
